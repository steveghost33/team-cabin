...
export default function PizzaGame() {
  ...
  useEffect(() => {
    ...
    function mkBld(x) {
      ...
      return { x, w: 50+Math.random()*90, h: 70+Math.random()*160, color: cols[Math.floor(Math.random()*4)], wc: Math.floor(Math.random()*4)+2, wr: Math.floor(Math.random()*3)+2 };
    }
    ...
    function spawnPizza() {
      ...
      pizzas.push({ x:W+scrollX+80, y:fly?GROUND-PH-50-Math.random()*60:GROUND-PH-4, bob:Math.random()*Math.PI*2, collected:false });
    }
    ...
    function drawBld(b, cx) {
      ...
      for (let r=0;r<b.wr;r++) for (let c=0;c<b.wc;c++) {
        ...
        if (wx+10<bx+b.w-4) { ctx.fillStyle=Math.sin(frame*0.013+r*1.8+c*0.9+b.x*0.01)>0?C.gold:'#091505'; ctx.fillRect(wx,wy,10,10); }
      }
    ...
    function drawPizza(pz) {
      const ox=pz.x-scrollX, bob=Math.sin(frame*0.08+pz.bob)*6, py=pz.y+bob;
      ...
      ctx.fillStyle='#FFE680'; [[10,10],[18,10],[14,16],[8,15],[19,17]].forEach(([dx,dy])=>ctx.fillRect(ox+dx-1,py+dy-1,3,3));
      ctx.fillStyle='#8B0000'; [[9,9],[18,8],[12,16],[20,14],[14,11]].forEach(([dx,dy])=>{ctx.beginPath();ctx.arc(ox+dx,py+dy,3,0,Math.PI*2);ctx.fill();});
      ctx.fillStyle='rgba(255,80,80,0.35)'; [[9,8],[18,7],[12,15]].forEach(([dx,dy])=>{ctx.beginPath();ctx.arc(ox+dx-1,py+dy-1,1.5,0,Math.PI*2);ctx.fill();});
      ctx.fillStyle='#B87008'; [[4,14],[24,14],[14,4],[7,7],[21,7],[7,21],[21,21]].forEach(([dx,dy])=>ctx.fillRect(ox+dx-1,py+dy-1,3,3));
    ...
    function drawTitle() {
      ...
      if(Math.floor(frame/25)%2===0){ctx.fillStyle=C.greenL;ctx.font='11px "Press Start 2P"';ctx.fillText('[ PRESS ENTER TO CHOOSE PLAYER ]',W/2,H/2+72);}
      if(highSc>0){ctx.fillStyle=C.goldL;ctx.font='9px "Press Start 2P"';ctx.fillText('HIGH SCORE: '+highSc,W/2,H/2+108);}
    }
    ...
    function drawCharSelect() {
      ...
      const topOffsets=[10,18,10],spriteHeights=[40,48,40];
      chars.forEach((ch,i)=>{
    ...
    function drawWin() {
      ...
      if(Math.floor(frame/28)%2===0){ctx.fillStyle=C.goldL;ctx.font='11px "Press Start 2P"';ctx.fillText('PRESS ENTER TO PLAY AGAIN',W/2,H/2+90);}
    }
      ...
      if (gState==='playing') {
        ...
        pizzas=pizzas.filter(pz=>{
          ...
          if(pl.x<ox+28&&pl.x+PW>ox&&pl.y<pz.y+bob+28&&pl.y+PH>pz.y+bob){
            pz.collected=true;sc+=100;pc++;addParts(ox+14,pz.y+14,C.gold,16);
            if(pc>=WIN){if(sc>highSc)highSc=sc;music.pause();gState='win';}
            sync();return false;
          }
          return true;
        });
