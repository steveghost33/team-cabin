// ─────────────────────────────────────────────
//  game/constants.js
//  All tuning values for Detroit Pizza Quest
// ─────────────────────────────────────────────

export const W = 780;
export const H = 520;
export const GROUND = H - 80;
export const PW = 24;
export const PH = 32;
export const GRAVITY = 0.65;
export const JUMP_POWER = -13;
export const MOVE_SPEED = 4.2;
export const MAX_HP = 100;
export const HP_REGEN = 15;          // hearts restore this much
export const MAX_LIVES = 3;
export const PIZZA_TO_BOSS = 16;     // slices needed to trigger boss
export const SONG_FILE = '/kylesong.mp3';
export const SONG_VOLUME = 0.5;
export const GLD = '#E2A820';
export const GRN = '#1C3D12';
export const GRN2 = '#2D4A1E';
export const CREAM = '#F5F0DC';

// ── LEVEL CONFIGS ─────────────────────────────
// Level order: Ypsilanti → Ferndale → Detroit
export const LEVELS = [
  {
    id: 1,
    name: 'YPSILANTI',
    subtitle: 'Depot Town · 3:00 PM',
    mission: "GET TO GROVE STUDIOS",
    introQuip: "Practice started 20 minutes ago. You're still home.",
    skyTop: '#1a4a8a',
    skyBot: '#4a90cf',
    groundTop: '#3a7a28',
    groundColor: '#1a3a10',
    sidewalkColor: '#a19883',
    roadColor: '#3f3d3a',
    buildingCols: ['#8B4513','#6B3A2A','#9B5523','#7B4010'],
    windowColor: '#FFD700',
    hasSun: true,
    hasClouds: true,
    spawnRate: 120,
    enemySpeed: 1.4,
    enemyTypes: ['cone','metermaid','rat'],
    pizzaRate: 80,
    heartRate: 360,
    boss: 'landlord',
  },
  {
    id: 2,
    name: 'FERNDALE',
    subtitle: 'Nine Mile · 10:00 PM',
    mission: "GET TO THE ROCK SHOW",
    introQuip: "Doors at 10. It's 10:23. The opener is done. Go.",
    skyTop: '#1e0d3c',
    skyBot: '#3c1e62',
    groundTop: '#2a2032',
    groundColor: '#36303e',
    sidewalkColor: '#4b4657',
    roadColor: '#2c2833',
    buildingCols: ['#5a1e80','#6a2898','#481868','#622090'],
    windowColor: '#ff88cc',
    windowColor2: '#88ddff',
    hasNeon: true,
    hasStars: true,
    spawnRate: 75,
    enemySpeed: 2.2,
    enemyTypes: ['cone','metermaid','muscledude','biker','rat'],
    pizzaRate: 68,
    heartRate: 520,
    boss: 'recordexec',
  },
  {
    id: 3,
    name: 'DETROIT',
    subtitle: 'Downtown · 2:00 AM',
    mission: "RID THE RATS, SAVE THE CITY",
    introQuip: "Every slice in Detroit: gone. The Rat King did this. End him.",
    skyTop: '#123016',
    skyBot: '#2c5424',
    groundTop: '#2b2b2b',
    groundColor: '#232323',
    sidewalkColor: '#4c4a44',
    roadColor: '#33322f',
    buildingCols: ['#2a4a1e','#345826','#223d18','#3d6229'],
    windowColor: '#E2A820',
    unlitWindow: '#182a12',
    silhouetteColor: 'rgba(16,32,18,0.55)',
    hasMoon: true,
    hasStars: true,
    spawnRate: 50,
    enemySpeed: 3.0,
    enemyTypes: ['cone','metermaid','muscledude','rat','biker'],
    pizzaRate: 60,
    heartRate: 700,
    boss: 'ratking',
  },
];
